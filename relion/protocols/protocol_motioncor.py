...
class ProtRelionMotioncor(ProtAlignMovies, ProtRelionBase):
    ...
    Motion Correction (ProtRelionMotioncor) — User Manual
        Overview

        The Motion Correction protocol performs beam-induced motion correction
        for cryo-EM movies using the Relion implementation of motion correction
        algorithms. Its primary objective is to compensate for specimen drift
        and particle movement occurring during electron exposure, thereby
        improving image sharpness, contrast transfer estimation, and the overall
        quality of downstream reconstructions.

        In modern cryo-EM workflows, motion correction is one of the earliest
        and most critical preprocessing stages. Electron exposure causes gradual
        physical displacement of the sample, especially in vitrified specimens,
        leading to blurred micrographs if uncorrected. By aligning movie frames
        and compensating for these shifts, the protocol produces cleaner and
        more interpretable micrographs suitable for particle picking, CTF
        estimation, classification, and high-resolution refinement.

        Inputs and General Workflow

        The protocol operates on dose-fractionated movies collected by direct
        electron detectors. Each movie contains multiple sequential frames that
        represent the temporal evolution of the exposure. The protocol aligns
        these frames to estimate and compensate for specimen motion before
        generating corrected summed micrographs.

        Biological users commonly apply this protocol immediately after data
        import. Correct motion correction substantially improves the visibility
        of high-resolution features such as secondary structure elements,
        ligand densities, and flexible domains.

        The workflow supports both standard movie formats and Electron Event
        Representation (EER) data generated by Falcon detectors. EER processing
        allows highly flexible temporal fractionation, enabling users to adapt
        frame grouping according to dose accumulation and specimen sensitivity.

        Frame Selection and Summation

        The protocol allows users to define which frames contribute to the final
        corrected average. Early frames may contain stronger beam-induced motion,
        while later frames may suffer from accumulated radiation damage.
        Selecting an appropriate frame range therefore has direct biological
        consequences.

        In many practical datasets, excluding the earliest frames can improve
        stability because these frames often exhibit the largest drift. At the
        same time, excluding too many late frames may reduce total signal and
        compromise particle visibility. The optimal balance depends on specimen
        stability, ice thickness, total dose, and detector performance.

        Dose Weighting

        Dose weighting is one of the most important features of this protocol.
        Radiation damage progressively destroys high-resolution information as
        exposure accumulates. Dose weighting compensates for this effect by
        reducing the contribution of heavily damaged frames during averaging.

        For most high-resolution cryo-EM projects, dose weighting should remain
        enabled. It improves the preservation of fine structural details while
        maintaining overall signal quality. Non-dose-weighted sums may still be
        useful for specific preprocessing tasks such as CTF estimation in some
        workflows.

        From a biological perspective, dose weighting is especially valuable
        for fragile complexes, membrane proteins, and radiation-sensitive
        specimens where high-resolution information decays rapidly during
        exposure.

        Global and Local Motion Correction

        The protocol supports both global and local motion correction strategies.
        Global correction assumes that the entire micrograph moves coherently,
        while local correction divides the image into smaller patches that can
        move independently.

        Global correction is generally sufficient for highly stable specimens or
        rapid exploratory processing. However, many cryo-EM datasets exhibit
        anisotropic local deformation caused by beam interaction with the ice.
        In these situations, local correction substantially improves high-
        resolution recovery.

        The number of patches determines the granularity of local motion
        estimation. Increasing the patch count allows more flexible correction
        but also increases computational complexity and may become unstable in
        low-signal datasets. For large particles or highly tilted ice, local
        correction often provides significant improvements.

        Frame Grouping and Temporal Resolution

        The protocol allows grouping multiple movie frames before estimating
        motion trajectories. Grouping improves signal-to-noise ratio during
        alignment and may stabilize correction for noisy datasets. However,
        excessive grouping reduces temporal resolution and may hide rapid early
        motion.

        In EER datasets, grouping is particularly important because detectors
        produce extremely fine temporal sampling. Biological users typically
        choose grouping values that correspond to reasonable dose accumulation
        per fraction, often balancing computational efficiency with accurate
        motion tracking.

        Gain References and Detector Defects

        The protocol supports gain correction and detector defect compensation.
        Gain references correct systematic detector sensitivity variations,
        ensuring consistent intensity scaling across the image. Proper gain
        correction is essential for accurate downstream processing and reliable
        high-resolution refinement.

        Detector defect correction identifies damaged or unstable pixels that
        could otherwise introduce artifacts into the corrected micrographs.
        This becomes especially important in long data collection campaigns or
        when using aging detectors.

        Binning and Data Scaling

        Spatial binning can be applied during processing to reduce image size
        and computational cost. This is particularly useful for super-resolution
        acquisitions or rapid preliminary screening.

        Although binning accelerates processing and reduces storage demands, it
        also limits the achievable resolution. For final high-resolution
        refinement projects, users generally prefer minimal or no binning unless
        the original sampling is excessively fine relative to the target
        resolution.

        Power Spectra and Quality Assessment

        The protocol can generate summed power spectra for downstream CTF
        estimation and quality control. Well-defined Thon rings in the power
        spectrum indicate successful motion correction and good data quality.

        Additional visualization products such as alignment plots and micrograph
        thumbnails facilitate rapid inspection of processing quality. These
        diagnostic outputs are particularly useful during data collection, where
        immediate feedback can guide acquisition decisions.

        Motion Trajectories and Interpretation

        The protocol records frame alignment trajectories and cumulative motion
        statistics. These measurements help users evaluate specimen stability,
        stage performance, and ice behavior.

        Excessive motion may indicate poor vitrification, charging effects,
        unstable support films, or problematic acquisition conditions. By
        examining motion trajectories, users can identify problematic datasets
        before investing significant computational resources in downstream
        refinement.

        Outputs and Their Interpretation

        The protocol produces aligned micrographs, optionally including both
        dose-weighted and non-dose-weighted versions. Corrected movies and
        associated motion metadata may also be retained for downstream analysis.

        Additional outputs can include power spectra, motion trajectory plots,
        detector defect information, and local motion parameters. Together,
        these outputs provide both corrected imaging data and valuable quality
        assessment information.

        Biologically, successful motion correction improves particle alignment,
        enhances classification stability, and enables more accurate
        interpretation of high-resolution structural features.

        Practical Recommendations

        For most cryo-EM datasets, users should begin with dose weighting
        enabled and modest local motion correction settings. Increasing patch
        granularity can improve results for flexible ice or large fields of
        view, but excessive patch subdivision may introduce instability in noisy
        data.

        EER datasets require particular attention to frame grouping and dose
        fractionation. Selecting fractions that correspond to biologically
        meaningful dose increments generally provides the best compromise between
        temporal resolution and signal quality.

        During data collection, users are encouraged to inspect alignment plots,
        cumulative motion values, and power spectra regularly. Early detection
        of unstable acquisition conditions can save substantial processing time
        and improve overall dataset quality.

        Final Perspective

        Motion correction is not simply a technical preprocessing step but a
        foundational stage that strongly influences the biological quality of
        all downstream cryo-EM analysis. Accurate correction of beam-induced
        motion preserves high-resolution information, improves map quality, and
        increases the reliability of structural interpretation. Careful choice
        of frame ranges, dose weighting strategies, and local correction
        parameters is essential for obtaining biologically meaningful
        reconstructions from modern cryo-EM datasets.
    """
