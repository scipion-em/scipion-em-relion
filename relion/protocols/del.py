...
def run_pipeline(opts):
    ...
    all_particles_resolution, all_particles_bfactor = get_postprocess_result(
        opts.input_postprocess_job + 'postprocess.star')
    ...
    while current_nr_particles <= opts.maximum_nr_particles and current_nr_particles < all_nr_particles:
        ...
        refine_job, already_had_it = addJob('Refine3D', refine_job_name, SETUP_CHECK_FILE, refine_options,
                                            refine3d_run_file, refine_alias)
        if not already_had_it:
        ...
        if halfmap_filename is not None:
            ...
            post_job, already_had_it = addJob('PostProcess', post_job_name, SETUP_CHECK_FILE, post_options,
                                              postprocess_run_file, post_alias)
            if not already_had_it:
            ...
            try:
                ...
                print(' RELION_IT: WARNING: Failed to get post-processed resolution for {} particles'.format(
                    current_nr_particles))
    ...
    for n_particles, resolution, pp_bfactor in zip(nr_particles, resolutions, pp_bfactors):
        ...
        print('{0:11d} {1:15.3f} {2:13.2f} {3:14.4f} {4:18.2f}'.format(n_particles, log_n_particles, resolution, inv_d2,
                                                                       -pp_bfactor))
    ...
    print(" RELION_IT: The fitted line is: Resolution = 1 / Sqrt(2 / {0:.3f} * Log_e(#Particles) + {1:.3f})".format(
        b_factor, intercept))
    ...
    for x in (1.5, 2, 4, 8):
        ...
        print(" RELION_IT:   {0:.2f} A from {1:d} particles ({2:d} % of the current number of particles)".format(
            resolution, current_nr_particles, int(x * 100)))
